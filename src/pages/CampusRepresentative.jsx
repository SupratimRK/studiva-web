...
// Icon mapping helper (simplified to use URLs)
const BenefitIcon = ({ src, alt }) => (
...
const CampusRepresentative = () => {
  ...
  const { contextSafe } = useGSAP(() => {
    ...
    // Refresh ScrollTrigger after a short delay to ensure everything is rendered
  ...
  const handleBenefitMouseMove = contextSafe((e) => {
    ...
    // Update CSS variables for the radial gradient shine effect
  ...
  const handleFormMouseMove = contextSafe((e) => {
    const target = e.currentTarget;
    const rect = target.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    target.style.setProperty('--form-mouse-x', `${x}px`);
    target.style.setProperty('--form-mouse-y', `${y}px`);
  });
  ...
  return (
    <div className="cr-page" ref={pageRef}>
      ...
      <section className="cr-form-section">
        <div className="cr-form-wrapper" onMouseMove={handleFormMouseMove}>
          {submitted ? (
            /* Success state */
